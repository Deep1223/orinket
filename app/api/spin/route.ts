...
export async function POST(req: Request) {
  ...
  const phone = typeof incoming.phone === "string" ? incoming.phone.trim() : ""

  if (!phone || (!email && !session_id)) {
    return jsonError("Phone and (email or session_id) are required", 400)
  }
  ...
  try {
    backendResponse = await fetch(`${origin}${ORINKET_BACKEND_PUBLIC.spin}`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Accept: "application/json" },
      body: JSON.stringify({ email, phone, session_id }),
      cache: "no-store",
    })
  } catch (e) {
