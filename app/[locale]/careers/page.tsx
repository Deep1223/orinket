...
import { careersPage, jobOpenings } from "@/data/dummyCompanyPages"
import { Briefcase, Sparkles, Check } from "lucide-react"
import Link from "next/link"
import { fonts } from "@/lib/fonts"

export const metadata: Metadata = {
  title: "Careers | ORINKET",
  description:
    "Join Orinket — design, retail, customer experience, and more. Demi-fine jewellery with a premium, people-first culture.",
}
      ...
      <main className="flex-1">
        <div className="max-w-6xl mx-auto px-4 py-10 sm:py-14">
          <div className="rounded-3xl border border-border bg-gradient-to-b from-cream to-white p-6 sm:p-10 shadow-sm">
            ...
            <div className="mt-12">
              <h2 className={`text-lg font-semibold text-foreground ${fonts.headings} tracking-wide`}>
                Open roles
              </h2>
              <p className={`mt-1 text-sm text-muted-foreground ${fonts.body}`}>
                Dummy listings for demo — send your CV and the role title to our talent inbox.
              </p>
                  ...
                  >
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className={`text-base font-semibold text-foreground ${fonts.headings}`}>
                          {job.title}
                        </h3>
                        <span className={`text-xs px-2 py-0.5 rounded-full bg-cream border border-border text-muted-foreground ${fonts.labels}`}>
                          {job.type}
                        </span>
                      </div>
                    ...
                    <a
                      href={`mailto:${storeContent.support.email}?subject=Application%20%E2%80%94%20${encodeURIComponent(job.title)}`}
            ...
            <div className="mt-10 rounded-2xl border border-border bg-cream p-6 text-center">
              <p className={`text-sm text-muted-foreground ${fonts.body}`}>
                General enquiries:{" "}
                <a
                  href={`mailto:${storeContent.support.email}`}
                  className="text-foreground font-medium hover:text-gold-dark"
                >
